...
int main(int argc, char* argv[]) {
  ...
  assert(CONSENSUS_OK == consensus_init());
  ...
  for(i = 0; i < count; i++) {
    ...
    assert(CONSENSUS_OK == consensus_generate_secret(secrets[i], hashes[i]));
    size_t j;
    printf("Secret:\t");
  ...
  puts("Check hashes");
  assert(CONSENSUS_OK == consensus_check_hashes(count, hashes));
  ...
  puts("Verify hashes");
  assert(CONSENSUS_OK == consensus_verify_secrets(count, hashes, secrets));

  uint8_t shared[CONSENSUS_SECRET_LENGTH];
  puts("Generate shared");
  // consensus_generate_shared bitwise XORs each secret together.
  assert(CONSENSUS_OK == consensus_generate_shared(count, secrets, shared));
